...
Vibe mode is a **TUI-only director mode**. `/vibe` (`src/slash-commands/builtin-modes.ts:233-250`) has only `handleTui` — no text-mode `handle` — so `executeAcpBuiltinSlashCommand` returns `false` for it (`acp-builtins.ts:66`) and the command is neither advertised (`acp-builtins.ts:37-38` filters handle-less specs) nor dispatched; a `/vibe` prompt goes to the model as literal text. `session/set_mode` offers only `default` + `plan` (`acp-agent.ts:1831-1842`; `#applyModeChange` throws on anything else, :1848-1851). Nothing outside `InteractiveMode` calls `activateVibeTools` (`interactive-mode.ts:4381-4443`), even though `createVibeTools` is wired into every top-level session including ACP ones (`sdk.ts:3774-3777`, `agent-session-types.ts:204-205`).
...
- **Entry point (blocker).** There is no ACP way to enter vibe mode — not `session/set_mode` (only default/plan), not `/vibe` (TUI-only `handleTui`), not a tool the model can call (vibe tools are installed only by `activateVibeTools`). The pager cannot drive vibe at all until OMP grows an ACP entry point. The pieces are all on `AgentSession` (`activateVibeTools` :5289, `deactivateVibeTools` :5294, `setVibeModeState` :5617, `getVibeModeState` :5613) plus `VibeSessionRegistry.ownerScope/activateScope/killAll` (`runtime.ts:361-377,1122+`) — the interactive-mode wrapper (`interactive-mode.ts:4381-4471`) is thin and portable. `vibe_spawn` also requires `session.asyncJobManager` (`runtime.ts:530-534`), which ACP sessions have (async delivery drain exists, `acp-agent.ts:1563-1575`).
...
1. **OMP-side prerequisite (driving):** add an ACP entry point — either a `handle` on the `/vibe` spec or a `"vibe"` arm in `#getAvailableModes`/`#applyModeChange` — that runs the `#enterVibeMode`/`#exitVibeMode` equivalent against `AgentSession` primitives (`activateVibeTools(["read","todo"])`, `VibeSessionRegistry.activateScope`, `setVibeModeState`, `killAll` + `deactivateVibeTools` on exit). Without this, everything below is observe-only for TUI-started sessions.
