...
Patch lives on branch `grok-omp/vibe-acp` in the OMP source clone (`~/Projects/Freelancing/personal/oh-my-pi`, upstream `github.com/can1357/oh-my-pi`), commit `9081533` on top of v18.1.17 (`3b3a6dc`). `main` stays clean — rebuilds must check out that branch. `packages/coding-agent/src/modes/acp/acp-agent.ts`:
...
Build: `cd ~/Projects/Freelancing/personal/oh-my-pi && git checkout grok-omp/vibe-acp && bun install && cp ~/.bun/install/global/node_modules/@oh-my-pi/pi-natives-darwin-arm64/pi_natives.darwin-arm64.node packages/natives/native/ && bun --cwd packages/coding-agent run build` → `packages/coding-agent/dist/omp` (needs the prebuilt `pi_natives` addon copied in, or `bun --cwd packages/natives run build` for a from-source build). The shipped `dist/omp` was built from `9081533`.

Verified: `bun bridge/acp-vibe-verify.mjs` (27 checks) against `dist/omp` — `session/new` lists `default,plan,vibe`; `set_mode`/`set_config_option` enter+exit vibe AND plan via both APIs; plan→vibe is rejected (mutual exclusion); vibe→plan exits vibe first; session JSONL records `mode_change:"vibe"` with the 14-tool `previousTools` snapshot. `bridge/pager-planmode-e2e.mjs` passes against the patched build (mode cycling always-approve→plan→auto, agent reaches idle).
