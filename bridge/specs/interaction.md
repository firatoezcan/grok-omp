...
- User questions: OMP's `ask` tool (`tools/ask.ts`), plan approval, and slash-command prompts (`omfg` save-location/overwrite/amend, `cleanse` picker, `goal` menu, `login` provider select) go through **standard ACP `elicit/create` (form mode)** when the client advertises `elicitation.form` (`acp-agent.ts:290-430`, `createAcpExtensionUiContext`); otherwise auto-approve/undefined. **The adapter injects `elicitation:{form:{},url:{}}` into `clientCapabilities`** during the initialize hygiene rewrite — the pager can't decode `elicitation/create` itself, but the adapter bridges it to the pager's private cards, so the capability is real.
...
4. IMPLEMENTED: agent→client `elicitation/create` requests are bridged — `mode:"form"` schemas map to `_x.ai/ask_user_question` (`enum`→single-select, `oneOf`/`anyOf` `{const,title,description}`→single-select, `array`+`items.enum`/`items.anyOf`→multi-select, boolean→Yes/No, bare string/number→freeform "Other" row; multi-property schemas become tabbed questions). OMP's `askDialog` `<key>__other` companion props are folded into the base question's built-in "Other" row and the typed text is written back under `__other` on submit; labels map back to their `const` when they differ. `mode:"url"` maps to `_x.ai/mcp/elicit`, and the plan-approval shape maps to `_x.ai/exit_plan_mode`. Responses translate back to `{action,content}`; non-accept outcomes become `cancel`/`decline`. `elicitation/complete` notifications translate to `_x.ai/mcp/elicit_complete` so the waiting URL card dismisses. Unrepresentable schemas fall through verbatim → method_not_found → OMP auto-approve.
