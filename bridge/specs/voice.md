...
- **Triggers**: `/voice` slash command, `Ctrl+Space` (toggle; hold-to-talk on Kitty-protocol terminals), `F8` (`ActionId::VoiceToggle`, `actions/defaults.rs:669-683`; `Action::EnableVoiceMode`/`VoiceToggle`/`VoiceStop`, `app/actions.rs:161-169`). Esc / recording-row `[stop]` stop capture.
